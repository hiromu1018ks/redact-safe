...
### 2026-04-22 - v2 Task 14: レイアウト解析結果がテキスト認識で活用されない問題を修正する

**変更内容:**
- `python-worker/ocr_pipeline.py`: `run_ocr_pipeline()` で図・画像（figure/image/picture）レイアウト領域を検出し、該当領域内のテキストリージョンをOCR結果から除外。除外した画像領域は `manual_mask_regions` として返し、手動マスキング確認対象としてマーク
- `python-worker/ocr_pipeline.py`: `run_ocr_pipeline()` の戻り値に `manual_mask_regions` を追加し、画像領域のbbox_pt（PDF座標）を含むメタデータを返す
- 表（table）領域のTesseractフォールバック優先はTask 13で実装済み
...
### 2026-04-22 - v2 Task 15: Undoのページ跨ぎ問題を修正し、一括操作のUndoを追加する

**変更内容:**
- `src/undo-manager.js`: `beginMacro(label)`/`endMacro()` メソッドを追加。マクロ記録中は `push()` がマクロの `ops` 配列に追加され、`endMacro()` で1つのUndoステップとしてスタックにプッシュされる
- `src/main.js`: `performUndo()` をリファクタリングし、マクロUndo対応と `undoSingleOp()` への分離を実装。`undoSingleOp()` で `op.pageNum` が現在ページと異なる場合、`pdfViewer.goToPage()` で自動的に該当ページに遷移してからUndoを実行
- `src/main.js`: `loadPdfWithAnalysis()` の先頭で `undoManager.clear()` を呼び出し、新しいPDF読込時にUndo履歴をリセット
- `src/main.js`: サイドバーの全てON/OFFボタンとツールバーの全てON/OFFボタンの両方で `beginMacro/endMacro` を使用。エラー時にも `endMacro()` を呼び出してマクロ状態をリセット

**実行コマンド:**
- `npm run build` - 成功
- `cargo clippy` - 成功 (dead_code warnings のみ、既存分)

**課題:** toggle undoでのenabled状態保存は既存のpersistToggleRegionで対応済み（トグル動作で元の状態に戻る）

### 2026-04-22 - v2 Task 16: 全モーダルダイアログにARIA属性・フォーカストラップ・Escapeハンドラを追加する

**変更内容:**
- `index.html`: 全6モーダルダイアログに `role="dialog"`, `aria-modal="true"`, `aria-labelledby` を追加
- `index.html`: メニューバーに `role="menubar"` を追加。各メニュートリガーに `role="menuitem"`, `aria-haspopup`, `aria-expanded` を追加
- `index.html`: `btn-close-debug` に `aria-label="閉じる"` を追加
- `src/main.js`: `trapFocus(container)` ユーティリティを実装し署名・確定警告ダイアログに適用
- `src/main.js`: メニュー開閉時に `aria-expanded` を更新
- `src/main.js`: 署名・確定実行者警告ダイアログにEscapeキーハンドラを追加
...
### 2026-04-22 - v2 Task 17: alert()をトースト通知に置き換え、サイドバー折りたたみ機能を追加する

**変更内容:**
- `src/main.js`: `showToast(message, type)` 関数を実装。`type`: error/warning/info で色分け。3秒後に自動消失（スライドアウトアニメーション）。`role="alert"` と `aria-live="assertive"` を付与
- `index.html`: `#toast-container` をbody直前に追加
- `src/main.js`: 全8箇所の `alert()` 呼び出しを `showToast()` に置き換え
- `index.html`: サイドバーヘッダーに折りたたみボタン `#btn-sidebar-toggle` を追加
- `index.html`: サイドバー展開ボタン `#btn-sidebar-expand` を追加
- `src/styles.css`: トースト通知のスタイル（画面右上固定、スライドイン/アウトアニメーション）を追加
- `src/styles.css`: サイドバー折りたたみ/展開のスタイルを追加
...
### 2026-04-22 - v2 Task 18: WCAG AA準拠のフォントサイズとコントラスト比を修正する

**変更内容:**
- `src/styles.css`: ステータスバーテキストを11px→12pxに変更
- `src/styles.css`: ステータスバッジを10px→11pxに変更
- `src/styles.css`: サイドバープレースホルダーの色を#888→#666に変更（コントラスト4.5:1以上を確保）
- `src/styles.css`: リージョンメタ情報(.region-meta)の色を#999→#666に変更
- `src/styles.css`: フィルターセレクトの`:focus`スタイルを`:focus-visible`に変更し、`outline: 2px solid #4a90d9`を追加
- `src/styles.css`: 全ボタン・セレクトに統一の`:focus-visible`スタイルを追加（outline: 2px solid #4a90d9）
...
### 2026-04-23 - v2 Task 19: main.jsを複数モジュールに分割し、重複するキーボードハンドラを統合する

**変更内容:**
- `src/ui/app-state.js`を新規作成：モジュール間共有状態（pdfViewer, maskingOverlay, undoManager等）を一元管理。isTauri, invoke, generateId, logAuditEvent, autoSaveDocumentも移動
- `src/ui/toast.js`を新規作成：showToast()関数とTOAST_TYPES定数を移動（34行）
- `src/ui/sidebar.js`を新規作成：PII_TYPE_LABELS, piiTypeLabel, サイドバーレンダリング・フィルタリング・一括ON/OFF・折りたたみを移動（260行）
- `src/ui/menu.js`を新規作成：メニューバーのドロップダウン・ホバー切替・ARIA expanded更新・メニューアイテムアクションを移動（100行）
- `src/ui/dialogs.js`を新規作成：trapFocus(), パスワード/署名/オペレータ/確定警告ダイアログ、設定ダイアログ（loadSettings/saveSettings/applySettings）、ヘルプダイアログを移動（437行）
- `src/main.js`をリファクタリング：3233行→2255行に削減。全モジュールからES importで関数をインポート。2つのkeydownイベントリスナーを1つに統合（Escape→メニュー閉じ→モーダル閉じ→リージョン選択解除の順序処理）。drag & dropのalert()をshowToast()に修正
- 各モジュールファイルが1000行以下であることを確認（最大437行: dialogs.js）

**実行コマンド:**
- `npm run build` - 成功 (18 modules transformed)
- `cargo clippy` - 成功 (dead_code warnings のみ、既存分)

**課題:** なし

### 2026-04-23 - v2 Task 20: PDFビューアのレンダリングキャンセルとfitToWidthのスクロールバー対応

**変更内容:**
- `src/pdf-viewer.js`: `renderPage()`で進行中のレンダリングを`RenderTask.cancel()`で即座にキャンセルするよう変更。`_currentRenderTask`プロパティでRenderTaskを追跡し、`RenderingCancelledException`は無視
- `src/pdf-viewer.js`: `fitToWidth()`でスクロールバー幅を考慮。`_estimateScrollbarWidth()`で親要素の実測値(`offsetWidth - clientWidth`)を使用し、スクロールバーがない場合は0、測定不可時は17pxフォールバック
- `src/masking-overlay.js`: `setHoveredRegion()`でdirty-region最適化を実装。前ホバーと新ホーブのリージョンのみ`_drawRegionDirty()`でbbox周辺をクリアして再描画（全キャンバス再描画を回避）
- `src/masking-overlay.js`: `_drawSelectionHandles()`でコーナーハンドル(8px)とエッジハンドル(4px)を視覚的に区別。`findHandleAtPoint()`でもコーナー(8px閾値)とエッジ(5px閾値)で異なるヒットエリアを設定し、コーナーを優先チェック

**実行コマンド:**
- `npm run build` - 成功 (18 modules transformed)
- `cargo clippy` - 成功 (dead_code warnings のみ、既存分)

**課題:** ブラウザでの動作確認は開発サーバーの起動権限がなかったためコードレビューで代替
...
### 2026-04-23 - v2 Task 21: PDF再オープン機能とbase64メモリ最適化を追加する

**変更内容:**

確認の結果、全ステップが既に実装済みであったため、コードレビューとビルド検証のみを実施。

- `src/main.js` — `openPdfFile()` で `showReopenConfirmDialog()` による確認ダイアログ表示が既に実装済み（`docStatusManager.getStatus()` が truthy の場合にダイアログを表示）
- `src/main.js` — `loadPdfWithAnalysis()` で `undoManager.clear()`、`appState.testRegions = []`、`appState.allRegionsByPage = {}`、`watermarkEl.style.display = "none"` による状態リセットが既に実装済み
- `src/main.js` — `base64ToUint8Array()` によるLUTベースの直接base64→Uint8Arrayデコードが既に実装済み（中間文字列を生成しない効率的な実装）
- `src/ui/sidebar.js` — `onSidebarRegionClick()` で `_pendingSidebarRegionId` パターンによる `onPageChange` コールバックベースのリージョン選択が既に実装済み（`setTimeout(200ms)` は使用されていない）
- `src/ui/dialogs.js` — `showReopenConfirmDialog()` がフォーカストラップ・ARIA属性付きで実装済み
- `index.html` — `#reopen-confirm-dialog` モーダルダイアログ要素が存在

**実行コマンド:**
- `npm run build` - 成功 (dist/ にビルド出力)

**課題:** ブラウザでの動作確認は権限未承認のため未実施。ビルド成功 + コードレビューで代替確認。

### 2026-04-23 - v2 Task 22: 改善内容のエンドツーエンド統合テストを実行する

**検証項目と結果:**

1. **cargo test** — 19 tests passed (ドキュメント状態管理・バックアップ・クラッシュリカバリー)
2. **cargo clippy** — 成功 (dead_code warnings のみ、既存分)
3. **npm run build** — 成功 (dist/ にビルド出力)
4. **マイナンバー・法人番号チェックデジット検証** — `validate_my_number()` / `validate_corporate_number()` が `pii_detector.py` で実装され `detect_pii()` から呼び出し済み
5. **Pythonワーカー自動再起動** — `ensure_worker_alive()` でプロセス生存チェック + 自動再起動が `lib.rs` に実装済み
6. **全モーダルのキーボード操作** — 7つのモーダル全てに `role="dialog" aria-modal="true" aria-labelledby` + `trapFocus()` + Escape/Enter ハンドラ
7. **Undoのページ跨ぎ操作** — `undoSingleOp()` で `pdfViewer.goToPage()` によるページ遷移後にUndo実行
8. **サイドバー折りたたみ・展開** — `collapsed` CSSクラス切替 + toggle/expand ボタンが実装済み
9. **ファイルパスベースIPC通信** — `finalize_masking_pdf` が `pdf_path` パラメータ使用（base64廃止）
10. **100MBサイズ制限** — `read_file_as_base64` / `save_base64_to_file` / `load_from_file` 全てに100MB制限
...
**課題:** デジタルPDFのエンドツーエンドテスト（ファイル読込→PII検出→仮マスキング→確定→安全PDF出力）と100MB PDFのOOMテストはTauri環境+Python環境が必要のため未実施。Pythonワーカーkill後の自動再起動テストも同様。
